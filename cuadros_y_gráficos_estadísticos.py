...
Original file is located at
    https://colab.research.google.com/drive/14veo70kTsw1bjmeGVLuaI79sd-PxFpWN
...
# Datos de porcentajes de juegos ganados (Pct)
pct = [
...
# a) Clasificación en cinco clases de igual tamaño
bins = [0.125, 0.275, 0.425, 0.575, 0.725, 0.875]

# b) Frecuencias absolutas y relativas
freq_abs, _ = np.histogram(pct, bins=bins)
freq_rel = freq_abs / len(pct)

# Mostrar frecuencias
print("Frecuencias Absolutas:", freq_abs)
print("Frecuencias Relativas:", freq_rel)

# c) Polígono de frecuencias
midpoints = [(bins[i] + bins[i+1]) / 2 for i in range(len(bins)-1)]
plt.plot(midpoints, freq_abs, marker='o', linestyle='-', color='b')
plt.title("Polígono de Frecuencias")
plt.xlabel("Puntos Medios de Clases")
plt.ylabel("Frecuencia Absoluta")
plt.grid(True)
plt.show()

# d) Ojiva "mayor que"
freq_acum = np.cumsum(freq_abs[::-1])[::-1]  # Frecuencias acumuladas "mayor que"
plt.plot(bins[:-1], freq_acum, marker='o', linestyle='-', color='r')
plt.title("Ojiva de Frecuencias Acumuladas (Mayor Que)")
plt.xlabel("Límites Inferiores de Clases")
plt.ylabel("Frecuencia Acumulada")
plt.grid(True)
plt.show()